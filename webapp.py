...
.em-alert { background: #FFF8E7; border: 1.5px solid #F5D78E; color: #7A5B00; border-radius: 14px; padding: 13px 16px; font-size: 14px; margin: 14px 0 20px; line-height: 1.8; }
...
.em-card { background: #FFFFFF; border: 1.5px solid #D7E7FA; border-radius: 18px; padding: 22px 18px; text-align: center; }
.em-card .em-ic { font-size: 34px; }
.em-card h3 { font-size: 16px; color: #0B2E6B; margin: 10px 0 4px; }
...
.em-call.blue { background: #1677E8; }
...
.em-mini-card { display: flex; align-items: center; gap: 10px; background: #F7FAFF; border: 1px solid #D7E7FA; border-radius: 14px; padding: 14px 16px; flex-wrap: wrap; }
.em-mini-card .em-mini-num { margin-inline-start: auto; font-size: 22px; font-weight: 800; color: #1677E8; }
...
.em-24h { display: inline-block; background: #E8F3FF; color: #1677E8; font-weight: 700; font-size: 13px; padding: 7px 16px; border-radius: 999px; }
.em-safety { background: #FFF8E7; border-color: #F5D78E; color: #7A5B00; }
...
L = {
    "ar": {
        ...
        "home_desc": "أدخل أعراضك في خطوات بسيطة واحصل على تقييم أولي ذكي يساعدك على فهم حالتك ومعرفة الخطوة المناسبة التالية — مع مصادر طبية موثوقة وبخصوصية كاملة.",
        ...
        "home_less": "عرض أقل",
        "home_how": "كيف يعمل؟",
        "home_step1_t": "أدخل معلوماتك",
        "home_step1_p": "العمر، الجنس، الأعراض، المدة، والشدة بأزرار بسيطة.",
        "home_step2_t": "تحليل ذكي",
        "home_step2_p": "محرك ذكاء اصطناعي يقيّم حالتك من مصادر طبية موثوقة.",
        "home_step3_t": "احصل على التقييم",
        "home_step3_p": "الاحتمالات المحتملة، مستوى الخطورة، والخطوة المناسبة التالية.",
    ...
    "en": {
        ...
        "home_desc": "Enter your symptoms in a few simple steps and get an initial smart assessment that helps you understand your condition and know the right next step — backed by trusted medical sources with full privacy.",
        ...
        "home_step1_t": "Enter your info",
        "home_step1_p": "Age, gender, symptoms, duration, and severity with simple buttons.",
        "home_step2_t": "Smart analysis",
        "home_step2_p": "An AI engine evaluates your case from trusted medical sources.",
        "home_step3_t": "Get your assessment",
        "home_step3_p": "Likely conditions, urgency level, and the right next step.",
...
def _nav():
    ...
    html += ('<div class="dd"><button class="dd-btn" onclick="toggleDD(event)">%s <span style="font-size:11px;">▼</span></button>'
             ...
             '</div></div>') % (_t("nav_explore"), _t("nav_q"), _t("nav_geo"),
                                 _t("nav_aware"), _t("nav_blog"))
    html += '</div>'
    html += ('<div class="lang-sw"><a href="#" onclick="setLang(&#39;ar&#39;);return false;" class="%s">عربي</a>'
...
WELCOME_LANGS = [
    ...
    ("🇨🇳", "中文", "zh"), ("🇩🇪", "Deutsch", "de"), ("🇵🇹", "Português", "pt"), ("🇷🇺", "Русский", "ru"),
    ("🇯🇵", "日本語", "ja"), ("🇰🇷", "한국어", "ko"), ("🇮🇳", "हिन्दी", "hi"), ("🇹🇷", "Türkçe", "tr"),
...
.phone-heart { width: 74px; height: 74px; border-radius: 50%; background: #FFFFFF; box-shadow: 0 10px 26px rgba(23,105,224,.30); display: flex; align-items: center; justify-content: center; font-size: 36px; margin-bottom: 18px; }
.phone-screen p { color: #0B2E6B; font-size: 15px; font-weight: 600; line-height: 1.7; }
.lang-section { padding: 40px 20px 50px; text-align: center; }
.lang-section h2 { color: #0B2E6B; font-size: 28px; margin-bottom: 4px; }
.lang-section .muted { color: #64748b; font-size: 15px; margin-bottom: 26px; }
.lang-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 14px; max-width: 900px; margin: 0 auto; }
.lang-tile { display: flex; align-items: center; gap: 12px; justify-content: center; background: #FFFFFF; border: 1.5px solid #D7E7FA; border-radius: 16px; padding: 15px 10px; font-family: inherit; font-size: 15px; font-weight: 600; color: #17356D; cursor: pointer; box-shadow: 0 3px 12px rgba(23,105,224,.06); transition: transform .15s ease, background .15s ease, color .15s ease, box-shadow .15s ease; }
.lang-tile .fl { font-size: 22px; }
.lang-tile:hover { background: #1769E0; color: #FFFFFF; transform: translateY(-3px); box-shadow: 0 12px 26px rgba(23,105,224,.32); }
...
def welcome_page():
    tiles = "".join(
        '<button class="lang-tile" onclick="ssGo(\'%s\')"><span class="fl">%s</span>%s</button>'
        % (code, flag, label)
        for flag, label, code in WELCOME_LANGS
    )
    body = """
...
def home_page():
    ...
    """ % (
        ...
        t("home_c_t"), t("home_c_p"), t("home_e_t"), t("home_e_p"),
...
CHAT = {
    "ar": {
        ...
        "result_disclaimer": "هذا تقييم أولي مبني على المعلومات المدخلة فقط، ولا يُعد تشخيصًا طبيًا ولا يُغني عن استشارة الطبيب.",
    ...
    "en": {
        ...
        "result_disclaimer": "This is a preliminary assessment based only on the entered information. It is not a medical diagnosis and does not replace seeing a doctor.",
...
def chat_page():
    ...
    function renderResult(d) {
      ...
      h += '<div class="res-urg"><span class="pill2 ' + pcls + '">' + pill + '</span></div>';
      h += '<div class="res-disc">' + esc(TT('result_disclaimer')) + '</div>';
      h += '<div class="res-note">' + esc(d.personal_note) + '</div>';
      ...
      if (d.ml_predictions && d.ml_predictions.length) {
        ...
        d.ml_predictions.forEach(p => {
          ...
          h += '<div style="display:flex;justify-content:space-between;font-size:13.5px;margin-top:6px;"><span>' + esc(NAME(p, 'name_ar', 'name_en')) + '</span><b>' + pct + '%</b></div>';
      ...
      if (d.danger_signs) h += '<div class="rc-title">' + TT('danger') + '</div><div class="res-sec">' + esc(d.danger_signs) + '</div>';
      if (d.when_to_seek_care) h += '<div class="rc-title">' + TT('when') + '</div><div class="res-sec">' + esc(d.when_to_seek_care) + '</div>';
      if (d.home_care) h += '<div class="rc-title">' + TT('home_care') + '</div><div class="res-sec">' + esc(d.home_care) + '</div>';
...
CT = {
    "ar": {
        ...
        "blood_sub": "ارفع صورة فحص الدم أو ملف PDF، وسنستخرج القيم ونفسّرها لك بوضوح.",
        "blood_gender": "الجنس",
        "blood_female": "أنثى", "blood_male": "ذكر", "blood_child": "طفل",
        "blood_age": "العمر",
        "blood_age_ph": "مثال: 5",
        "blood_hint": "يُستخدم العمر والجنس لتحسين تفسير النتائج.",
        "blood_drop": "اسحب ملف الفحص هنا",
        "blood_drop_or": "أو",
        "blood_drop_btn": "📁 اختيار ملف",
        "blood_drop_note": "PDF • JPG • PNG • حتى 10 ميجابايت",
        "blood_file_del": "🗑️ حذف الملف",
        "blood_btn": "🔍 تحليل الفحص",
    ...
    "en": {
        ...
        "blood_cbc": "Complete Blood Count (CBC)",
        "blood_sub": "Upload a photo of your blood test or a PDF, and we'll extract the values and explain them clearly.",
        "blood_gender": "Gender",
        "blood_female": "Female", "blood_male": "Male", "blood_child": "Child",
        "blood_age": "Age",
        "blood_age_ph": "Example: 5",
        "blood_hint": "Age and gender are used to improve the interpretation of results.",
        "blood_drop": "Drag your test file here",
        "blood_drop_or": "or",
        "blood_drop_btn": "📁 Choose file",
        "blood_drop_note": "PDF • JPG • PNG • up to 10 MB",
        "blood_file_del": "🗑️ Remove file",
        "blood_btn": "🔍 Analyze Test",
        ...
        "meds_h": "💊 Search for a Medication",
        ...
        <div class="grid2">
          <div class="field-box">
            ...
            <div style="flex:1;">
              ...
              <select class="inp" id="bg"><option value="f">__BF__</option><option value="m">__BM__</option><option value="c">__BC__</option></select>
            </div>
          </div>
          <div class="field-box">
            <div class="fb-ic">🎂</div>
...
def tips_page():
    ...
    <div class="card" style="text-align:center;">
      <h2>__TIPSH__</h2>
      <div id="tipBox" style="font-size:17px;line-height:2;padding:20px;background:#F0F7FF;border-radius:12px;margin:14px 0;"></div>
...
def emergency_page():
    ...
    <div class="card" id="geo">
      ...
      <div id="geoMsg" style="text-align:center;margin-top:10px;font-weight:700;color:#1677E8;"></div>
