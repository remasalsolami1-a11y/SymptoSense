...
.welcome-full { position: fixed; inset: 0; z-index: 60; background: linear-gradient(180deg, #f6f9fb 0%, #e8f7f4 100%); overflow: hidden; cursor: pointer; transition: opacity .8s ease, transform .8s ease; }
.welcome-full.fade-out { opacity: 0; transform: scale(1.08); }
.w-cloud { position: absolute; inset: 0; }
.gword { position: absolute; display: inline-block; font-weight: 800; color: #0f766e; white-space: nowrap; text-shadow: 0 2px 14px rgba(13,148,136,.18); animation: floaty var(--dur,10s) ease-in-out var(--delay,0s) infinite; }
@keyframes floaty { 0%,100% { transform: rotate(var(--rot,0deg)) translateY(0); } 50% { transform: rotate(var(--rot,0deg)) translateY(-16px); } }
...
def welcome_page():
    words = []
    for _ in range(85):
        f, t = random.choice(GREETINGS)
        top = round(random.uniform(1, 91), 2)
        left = round(random.uniform(1, 88), 2)
        size = round(random.uniform(24, 62))
        rot = round(random.uniform(-18, 18))
        op = round(random.uniform(0.55, 1.0), 2)
        delay = round(random.uniform(0, 6), 2)
        dur = round(random.uniform(8, 15), 2)
        words.append(
            '<span class="gword" style="top:%s%%;left:%s%%;font-size:%spx;'
            '--rot:%sdeg;--delay:%ss;--dur:%ss;opacity:%s;">%s %s</span>'
            % (top, left, size, rot, delay, dur, op, f, t)
        )
    cloud = "".join(words)
    body = """
