...
def landing_page():
    ...
    <div class="features">
      <a class="feature serv" href="/chat"><div class="ic">🩺</div><h3>فحص الأعراض</h3><p>أدخلي أعراضك واحصلي على تقييم أولي ذكي مع خطورة الحالة (بسيط / موعد / طوارئ).</p></a>
      <a class="feature serv" href="/blood"><div class="ic">🩸</div><h3>تحليل فحص الدم</h3><p>ارفعي صورة أو PDF لتحليل الدم (CBC) واحصلي على تفسير القيم والمؤشرات.</p></a>
      ...
      <a class="feature serv" href="/chat"><div class="ic">🏥</div><h3>أقرب مستشفى</h3><p>بناءً على موقعك، نعرض لك أقرب المرافق الصحية بالمسافة ورابط الخريطة.</p></a>
      <a class="feature serv" href="/chat"><div class="ic">❓</div><h3>أسئلة لطبيبك</h3><p>أسئلة ذكية جاهزة تسأليها لطبيبك في الموعد، مع علامات الخطر ومتى تراجعين.</p></a>
...
def chat_page():
    ...
    <div class="chat-wrap">
      <div class="chat-head">
        ...
        <button id="spkBtn" class="spk-btn" onclick="toggleSpeak()" title="تشغيل/إيقاف القراءة الصوتية">🔊 قراءة: مفعلة</button>
      </div>
    ...
    function toggleSpeak() {
      ...
      if (b) b.textContent = autoSpeak ? '🔊 قراءة: مفعلة' : '🔇 قراءة: متوقفة';
    }
    function speakText(txt) {
      ...
      uu.lang = 'ar-SA';
      const v = speechSynthesis.getVoices().find(v => v.lang && v.lang.toLowerCase().indexOf('ar') === 0);
      if (v) uu.voice = v;
      uu.rate = 0.95;
    ...
    function askSymptoms() {
      state.step = 'symptoms';
      add(G(
        'ما هي أعراضك؟ اضغطي على الأعراض التي تشعرين بها (يمكنك اختيار أكثر من واحد). وإذا لم تجدي ما تشعرين به، اكتبيه في صندوق الكتابة. عند الانتهاء اضغطي: ✅ انتهيت',
        'ما هي أعراضك؟ اضغط على الأعراض التي تشعر بها (يمكنك اختيار أكثر من واحد). وإذا لم تجد ما تشعر به، اكتبه في صندوق الكتابة. عند الانتهاء اضغط: ✅ انتهيت'
      ), 'bot');
      ...
      items.push({label:'✍️ اكتب عرضاً بنفسك', fn:()=>{
        add(G('لم تجدي ما تشعرين به؟ اكتبيه هنا:', 'لم تجد ما تشعر به؟ اكتبه هنا:'), 'bot');
    ...
    function renderResult(d) {
      ...
      addHtml('<div style="margin-top:10px;text-align:center;"><button class="opt" onclick="speakResult()">🔊 استمع للتحليل كاملاً</button></div>', 'result');
    ...
    function speakResult() {
      ...
      const pill = u==='high' ? 'طوارئ' : (u==='medium' ? 'يحتاج موعد طبيب' : 'بسيط');
      let parts = [];
      parts.push('نتيجة التحليل: الخطورة ' + pill + '.');
      if (d.personal_note) parts.push(clean(d.personal_note));
      if (d.possible_conditions) parts.push('الاحتمالات المحتملة: ' + clean(d.possible_conditions));
      if (d.recommendations && d.recommendations.length) {
        parts.push('التوصيات:');
        d.recommendations.forEach(r => { if (r.tip) parts.push('- ' + clean(r.tip)); });
      }
      if (d.med_warnings && d.med_warnings.length) {
        parts.push('تحذيرات الأدوية:');
        d.med_warnings.forEach(m => { if (m.warning_ar) parts.push('- ' + clean(m.warning_ar)); });
      }
      if (d.danger_signs) parts.push('علامات الخطر: ' + clean(d.danger_signs));
      if (d.when_to_seek_care) parts.push('متى تراجع الطبيب: ' + clean(d.when_to_seek_care));
      if (d.home_care) parts.push('الرعاية المنزلية: ' + clean(d.home_care));
      if (d.medication_guidance) parts.push('إرشاد الدواء: ' + clean(d.medication_guidance));
      if (d.questions_for_doctor) parts.push('أسئلة اسأل طبيبك: ' + clean(d.questions_for_doctor));
