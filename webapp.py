...
.welcome-full { position: fixed; inset: 0; z-index: 60; background: linear-gradient(180deg, #f6f9fb 0%, #e8f7f4 100%); overflow: hidden; transition: opacity .8s ease; }
.welcome-full.fade-out { opacity: 0; }
.w-cloud { position: absolute; inset: 0; }
.gword { position: absolute; display: inline-block; font-family: 'Amiri', 'Cairo', serif; font-weight: 700; color: #0f766e; white-space: nowrap; text-shadow: 0 3px 18px rgba(13,148,136,.20); animation: floaty var(--dur,10s) ease-in-out var(--delay,0s) infinite; }
...
.lang-btn { font-family: 'Cairo', 'Segoe UI', sans-serif; font-size: 15px; font-weight: 700; color: #0f766e; background: rgba(255,255,255,.9); border: 2px solid #0f766e; border-radius: 999px; padding: 10px 30px; cursor: pointer; box-shadow: 0 4px 16px rgba(13,148,136,.18); transition: transform .15s ease, background .15s ease; }
.lang-btn:hover { transform: translateY(-2px); background: #ccfbf1; }
.exit-curtain { position: fixed; z-index: 999; left: 50%; top: 50%; width: 150vmax; height: 150vmax; margin-left: -75vmax; margin-top: -75vmax; border-radius: 50%; background: radial-gradient(circle at center, #14b8a6, #0f766e 60%, #115e59); transform: scale(0); opacity: 0; pointer-events: none; transition: transform .9s cubic-bezier(.65,0,.35,1), opacity .55s ease; }
...
def _page(title, body, desc=None, bare=False):
    if not desc:
        desc = _t("desc")
    lang = _lang()
    base = _site_url()
...
WELCOME_WORDS = [
    ("🇸🇦", "أهلاً بك"), ("🇸🇦", "مرحباً"), ("🇸🇦", "حياك الله"),
    ("🇬🇧", "Welcome"), ("🇬🇧", "Hello"), ("🇹🇷", "Hoş geldiniz"),
    ("🇫🇷", "Bienvenue"), ("🇪🇸", "Bienvenido"), ("🇩🇪", "Willkommen"),
    ("🇮🇹", "Benvenuto"), ("🇵🇹", "Bem-vindo"), ("🇳🇱", "Welkom"),
    ("🇮🇩", "Selamat datang"), ("🇲🇾", "Selamat datang"), ("🇵🇱", "Witaj"),
    ("🇸🇪", "Välkommen"), ("🇫🇮", "Tervetuloa"), ("🇷🇴", "Bine ai venit"),
    ("🇻🇳", "Chào mừng"), ("🇦🇿", "Xoş gəldiniz"),
]

# anchor points spread across all parts of the screen (top%, left%)
WELCOME_ZONES = [
    (5, 7), (4, 40), (6, 74), (14, 88), (22, 12), (20, 60), (28, 26),
    (34, 84), (42, 8), (40, 44), (38, 72), (52, 90), (56, 18), (54, 50),
    (62, 78), (70, 12), (68, 40), (74, 62), (82, 30), (84, 80),
]


def welcome_page():
    words = []
    for i, ((f, t), (zt, zl)) in enumerate(zip(WELCOME_WORDS, WELCOME_ZONES)):
        size = random.randint(38, 66)
        top = max(2, min(90, zt + random.randint(-4, 4)))
        left = max(2, min(90, zl + random.randint(-5, 5)))
        rot = random.randint(-8, 8)
        delay = round(random.uniform(0, 4), 2)
        dur = round(random.uniform(9, 14), 2)
        words.append(
            '<span class="gword" style="top:%s%%;left:%s%%;font-size:%spx;'
            '--rot:%sdeg;--delay:%ss;--dur:%ss;">%s %s</span>'
            % (top, left, size, rot, delay, dur, f, t)
        )
    cloud = "".join(words)
    body = """
    <div class="welcome-full" id="wSt1">
      <div class="w-cloud">""" + cloud + """</div>
      <div class="lang-area">
        <div class="lang-opts" id="langOpts">
          <button class="pick-btn" onclick="ssGo('ar')">🇸🇦 عربي</button>
          <button class="pick-btn" onclick="ssGo('en')">English 🇬🇧</button>
        </div>
        <button class="lang-btn" id="langBtn" onclick="toggleLangs()">🌐 __LANGBTN__</button>
      </div>
    </div>
    ...
    function toggleLangs() {
      var o = document.getElementById('langOpts');
    ...
    """
    body = body.replace("__LANGBTN__", _t("lang_btn"))
    return _page(_t("title_landing"), body, bare=True)
