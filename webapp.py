...
.welcome-full { position: fixed; inset: 0; z-index: 60; background: linear-gradient(180deg, #f6f9fb 0%, #e8f7f4 100%); overflow: hidden; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: opacity .8s ease, transform .8s ease; }
.welcome-full.fade-out { opacity: 0; transform: scale(1.08); }
.w-cloud { max-width: 1080px; width: 94%; padding: 8vh 4vw; display: flex; flex-wrap: wrap; align-items: center; justify-content: center; gap: 12px 26px; }
.gword { display: inline-block; font-weight: 800; color: #0f766e; white-space: nowrap; text-shadow: 0 2px 16px rgba(13,148,136,.18); animation: floaty var(--dur,10s) ease-in-out var(--delay,0s) infinite; }
@keyframes floaty { 0%,100% { transform: rotate(var(--rot,0deg)) translateY(0); } 50% { transform: rotate(var(--rot,0deg)) translateY(-14px); } }
.tap-hint { position: absolute; bottom: 26px; left: 0; right: 0; text-align: center; color: #0f766e; font-weight: 700; font-size: 15px; animation: hintPulse 2.2s ease-in-out infinite; transition: opacity .5s ease; }
...
WELCOME_WORDS = [
    ...
    ("🇹🇷", "Hoş geldiniz"), ("🇫🇷", "Bienvenue"), ("🇪🇸", "Bienvenido"),
    ("🇩🇪", "Willkommen"), ("🇮🇹", "Benvenuto"), ("🇵🇹", "Bem-vindo"),
    ("🇳🇱", "Welkom"), ("🇮🇩", "Selamat datang"), ("🇲🇾", "Selamat datang"),
    ("🇵🇱", "Witaj"), ("🇸🇪", "Välkommen"), ("🇫🇮", "Tervetuloa"),
    ("🇷🇴", "Bine ai venit"), ("🇻🇳", "Chào mừng"),
]


def welcome_page():
    words = []
    for f, t in WELCOME_WORDS:
        size = random.randint(34, 72)
        rot = random.randint(-6, 6)
        op = round(random.uniform(0.85, 1.0), 2)
        delay = round(random.uniform(0, 5), 2)
        dur = round(random.uniform(9, 14), 2)
        words.append(
            '<span class="gword" style="font-size:%spx;--rot:%sdeg;--delay:%ss;'
            '--dur:%ss;opacity:%s;">%s %s</span>'
            % (size, rot, delay, dur, op, f, t)
        )
    cloud = "".join(words)
    body = """
    <div class="welcome-full" id="wSt1">
      <div class="w-cloud">""" + cloud + """</div>
      <div class="tap-hint" id="tapHint">👆 __TAPHINT__</div>
    </div>
    <div class="welcome-pick" id="wSt2">
      <div class="logo-big">🏥</div>
      <h1>""" + _t("w_title") + """</h1>
      <p class="muted" style="margin-bottom:20px;">""" + _t("w_pick") + """</p>
      <div class="lang-row">
    ...
    document.getElementById('wSt1').addEventListener('click', function(){
      if (document.getElementById('wSt2').classList.contains('show')) return;
      document.getElementById('wSt1').classList.add('fade-out');
      var hint = document.getElementById('tapHint');
      if (hint) hint.style.opacity = '0';
      setTimeout(function(){
        document.getElementById('wSt1').style.display = 'none';
        document.getElementById('wSt2').classList.add('show');
      }, 750);
    });
    </script>
    """
    body = body.replace("__TAPHINT__", _t("w_taphint"))
