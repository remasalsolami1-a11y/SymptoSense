...
<script>
let symptomsChart, langChart, urgencyChart, ageChart;
...
def api_stats():
    ...
    c.execute("SELECT urgency, COUNT(*) FROM records GROUP BY urgency")
    urgency = {row[0]: row[1] for row in c.fetchall()}

    # Language distribution
    c.execute("SELECT lang, COUNT(*) FROM records GROUP BY lang")
    lang = {row[0]: row[1] for row in c.fetchall()}

    # Age groups
    c.execute("SELECT age FROM records WHERE age IS NOT NULL")
