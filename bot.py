...
def _md_safe(text, lang: str = "ar") -> str:
    """Strip Markdown-breaking characters AND any stray foreign-script words
    the LLM occasionally leaks (e.g. Spanish/Vietnamese/Hebrew mixed into an
    Arabic response). The response should be monolingual, so anything from
    the 'wrong' script is noise, not content."""
    if not text:
        return text
    text = str(text)
    if lang == "ar":
