...
async def analysis_inline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("cond_"):
        if data == "cond_typing":
            ...
            return CONDITIONS
        cond = _flat_conds(tx)[int(data[5:])]
...
async def analyze_symptoms(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        logger.warning(f"DB last record failed: {db_err}")
...
{{"personal_note":"one or two empathetic, personalized sentences addressing the patient directly based on their specific situation (not generic)","urgency":"low|medium|high","urgency_text":"Simple|Needs appointment|Emergency","confidence":"high|medium|low","possible_conditions":"Possible conditions in English only (3 sentences, no definitive diagnosis)","recommendations":[{{"tip":"tip in English","source":"source name like Mayo Clinic","source_url":"https://..."}},{{"tip":"tip in English","source":"source name","source_url":"https://..."}},{{"tip":"tip in English","source":"source name","source_url":"https://..."}},{{"tip":"tip in English","source":"source name","source_url":"https://..."}}],"danger_signs":"Danger signs in English only","when_to_seek_care":"When to see a doctor in English only","home_care":"Home care tips in English only","medication_guidance":"cautious guidance about continuing medication or consulting a doctor/pharmacist, or empty if no medications mentioned","questions_for_doctor":"3-4 smart questions in English the patient should ask their doctor based on their case"}}"""
    ...
    try:
        logger.info(f"ANALYSIS START: user={user_id} lang={lang} symptoms={d.get('symptoms', [])}")
        client = Groq(api_key=os.environ["GROQ_API_KEY"])
        ...
        if len(result_text) > 4000:
            await _send(result_text[:2000])
            await _send(result_text[2000:], reply_markup=kb)
        else:
            await _send(result_text, reply_markup=kb)
        logger.info(f"ANALYSIS DONE: user={user_id} urgency={result.get('urgency')}")
        ...
        try:
            ...
            if chart:
                ...
                await msg.reply_photo(photo=chart, caption=caption)
        except Exception as chart_err:
        ...
        try:
            ...
            await _send_voice_summary(msg, context, spoken)
        except Exception as voice_err:
        ...
        await msg.reply_text(tx["followup_prompt"])
        ...
        try:
            ...
            await msg.reply_text(tx["feedback_title"], reply_markup=fb_kb)
        ...
        if result.get("urgency") == "high":
            ...
            await msg.reply_text(tx["share_location_prompt"], reply_markup=loc_kb)

    except Exception as e:
        logger.exception(f"ANALYSIS FAILED: user={user_id} error={type(e).__name__}: {e}")
        detail = f"{type(e).__name__}: {str(e)[:200]}"
        senders = []
        try:
            if update.effective_chat:
                senders.append(update.effective_chat.send_message)
        except Exception:
            pass
        if msg is not None:
            senders.append(msg.reply_text)
        sent = False
        for send in senders:
            try:
                await send(t(context, "error") + f"\n\n🛠 {detail}")
                sent = True
                break
            except Exception:
                continue
        if not sent:
            try:
                await msg.reply_text(t(context, "error"))
            except Exception:
                pass
...
async def _text_to_speech(text: str, lang: str) -> bytes:
    ...
    try:
        ...
        buffer = await asyncio.wait_for(_stream(), timeout=20)
        if buffer:
            return buffer
...
async def log_unhandled_error(update, context):
    logger.exception(f"UNHANDLED ERROR: {type(context.error).__name__}: {context.error}")
...
def main():
    ...
    if os.environ.get("START_WEBAPP_IN_BOT", "1") == "1":
        try:
            import webapp
            threading.Thread(target=webapp.run_webapp, daemon=True).start()
            logger.info("Web app started in background thread.")
        except Exception as dash_err:
            logger.warning(f"Web app failed to start (bot will continue normally): {dash_err}")
