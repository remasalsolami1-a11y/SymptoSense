...
TEXTS = {
    "ar": {
        ...
        "home_chronic_btn": "🔁 سجل حالة مزمنة",
        ...
        "chronic_ask_cond": "🔁 أي حالة تبي تسجلها اليوم؟",
        "chronic_sugar": "🍬 سكر",
        "chronic_pressure": "🩸 ضغط الدم",
        "chronic_asthma": "🌬️ ربو",
        "chronic_other": "📝 أخرى",
        "chronic_ask_reading": "أرسلي القراءة/التسجيل (مثال لسكر: 140 قبل الأكل)\nأو 🔙 للرجوع",
        "chronic_saved": "✅ تم تسجيل حالتك:\n{} — {}",
    ...
    "en": {
        ...
        "chronic_ask_cond": "🔁 Which condition do you want to log today?",
        "chronic_sugar": "🍬 Sugar",
        "chronic_pressure": "🩸 Blood Pressure",
        "chronic_asthma": "🌬️ Asthma",
        "chronic_other": "📝 Other",
        "chronic_ask_reading": "Send the reading/note (e.g., for sugar: 140 before meals)\nOr 🔙 to go back",
        "chronic_saved": "✅ Logged:\n{} — {}",
...
def _home_keyboard(context):
    ...
    share_url = f"https://t.me/{username}?start=share" if username else "https://t.me/"
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton(tx["home_meds_btn"], callback_data="home_meds"),
         InlineKeyboardButton(tx["home_chronic_btn"], callback_data="home_chronic")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(TEXTS[lang]["home_welcome"], reply_markup=_home_keyboard(context))
...
async def home_meds(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(tx["med_ask_name"], reply_markup=kb)

async def home_chronic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    lang = context.user_data.get("lang", "ar")
    tx = TEXTS[lang]
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(tx["chronic_sugar"], callback_data="chron_sugar")],
        [InlineKeyboardButton(tx["chronic_pressure"], callback_data="chron_pressure")],
        [InlineKeyboardButton(tx["chronic_asthma"], callback_data="chron_asthma")],
        [InlineKeyboardButton(tx["chronic_other"], callback_data="chron_other")],
        [InlineKeyboardButton(tx["home_back_btn"], callback_data="home_menu")],
    ])
    await query.message.reply_text(tx["chronic_ask_cond"], reply_markup=kb)

async def chronic_cond_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    lang = context.user_data.get("lang", "ar")
    tx = TEXTS[lang]
    cond_map = {"chron_sugar": "سكر", "chron_pressure": "ضغط الدم", "chron_asthma": "ربو", "chron_other": "أخرى"}
    context.user_data["chronic_cond"] = cond_map.get(query.data, "أخرى")
    context.user_data["_await"] = "chronic_reading"
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(tx["home_back_btn"], callback_data="home_menu")]])
    await query.message.reply_text(tx["chronic_ask_reading"], reply_markup=kb)
...
async def home_drug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(tx["home_back_btn"], callback_data="home_menu")]])
    await query.message.reply_text(tx["drug_ask_name"], reply_markup=kb)
...
async def handle_tool_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        elif await_flag == "chronic_reading":
            cond = context.user_data.get("chronic_cond", "أخرى")
            try:
                db.log_chronic(update.effective_user.id, cond, text)
            except Exception as e:
                logger.warning(f"Chronic log failed: {e}")
            context.user_data.pop("_await", None)
            await update.message.reply_text(tx["chronic_saved"].format(cond, text))
...
async def home_tips(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(tx["tips_menu_title"], reply_markup=kb)
...
async def home_followup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rec = None
    try:
        rec = db.get_last_record(update.effective_user.id)
...
async def home_firstaid(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(tx["firstaid_choose"], reply_markup=InlineKeyboardMarkup(rows))
...
async def home_relax(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = context.user_data.get("lang", "ar")
    tx = TEXTS[lang]
    await query.message.reply_text(tx["relax_title"] + "\n\n" + wellbeing.relax_guide(lang))
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(home_chronic, pattern="^home_chronic$"))
    app.add_handler(CallbackQueryHandler(chronic_cond_cb, pattern="^chron_"))
