...
اجب بـ JSON فقط بالعربية:
{{"urgency":"low|medium|high","urgency_ar":"بسيط|يحتاج موعد طبيب|طوارئ","possible_conditions":"الاحتمالات (3 جمل)","recommendations":["نصيحة1","نصيحة2","نصيحة3","نصيحة4"],"danger_signs":"علامات خطر","when_to_seek_care":"متى تراجع الطبيب","home_care":"رعاية منزلية"}}"""
    else:
        prompt = f"""You are a medical awareness assistant. Patient information:
...
Reply with JSON only in English:
{{"urgency":"low|medium|high","urgency_text":"Simple|Needs appointment|Emergency","possible_conditions":"Possible conditions (3 sentences)","recommendations":["tip1","tip2","tip3","tip4"],"danger_signs":"Danger signs","when_to_seek_care":"When to see a doctor","home_care":"Home care tips"}}"""
    ...
    try:
        ...
        if len(msg) > 4000:
            await update.message.reply_text(msg[:2000], parse_mode="Markdown")
            await update.message.reply_text(msg[2000:], parse_mode="Markdown", reply_markup=kb)
        else:
