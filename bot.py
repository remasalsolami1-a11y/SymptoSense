...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(welcome + "\n\n" + _menu_read_text(context), reply_markup=_home_keyboard(context))
...
async def home_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _reset_conversation(update, context)
    lang = context.user_data.get("lang", "ar")
    await query.message.reply_text(TEXTS[lang]["home_welcome"] + "\n\n" + _menu_read_text(context), reply_markup=_home_keyboard(context))
...
async def home_emergency(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    tx = TEXTS[lang]
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(tx["home_back_btn"], callback_data="home_menu")]])
    await query.message.reply_text(f"<b>{_html_escape(tx['emergency_title'])}</b>\n\n{_html_escape(tx['emergency_block'])}", parse_mode="HTML", reply_markup=kb)
...
async def home_voice_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(msg, reply_markup=_home_keyboard(context))
...
async def home_blood(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text(tx["blood_ask_gender"], reply_markup=kb)
    return BLOOD_GENDER
...
def _tts_plain(text: str, lang: str) -> str:
    """Strips HTML tags/entities so spoken text stays clean."""
    plain = re.sub(r"<[^>]+>", "", text)
...
async def _attach_speak_button(chat_id, text, reply_markup):
    ...
    rows = list(reply_markup.inline_keyboard) if isinstance(reply_markup, InlineKeyboardMarkup) else []
    return InlineKeyboardMarkup(rows + [[btn]])
...
async def _patched_send_message(self, chat_id, text=None, reply_markup=None, **kwargs):
    ...
    if isinstance(text, str) and text:
        try:
            reply_markup = await _attach_speak_button(chat_id, text, reply_markup)
...
async def voice_tool(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Lets users answer home-flow questions (med name/times, drug, feedback) by voice."""
    if not context.user_data.get("_await"):
        return
    try:
        text = await _transcribe_voice(update, context)
    except Exception as e:
