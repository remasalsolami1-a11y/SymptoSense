...
def add_subscriber(user_id, lang="ar"):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO subscribers (user_id, lang, subscribed, added_at) VALUES (?,?,1,?) "
        "ON CONFLICT(user_id) DO UPDATE SET lang=excluded.lang, subscribed=1",
...
def unsubscribe(user_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE subscribers SET subscribed=0 WHERE user_id=?", (user_id,))
    conn.commit()
    conn.close()
