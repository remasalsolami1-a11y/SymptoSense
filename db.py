...
def init_db():
    ...
    try:
        ...
        if USE_POSTGRES:
            ...
            """)
            c.execute("CREATE INDEX IF NOT EXISTS idx_chronic_user ON chronic_logs(user_hash)")
            c.execute("""
...
def export_all_records_xlsx():
    ...
    chronic_rows = _all_chronic_logs()
    if chronic_rows:
        ws3 = wb.create_sheet("Chronic Logs")
        ws3.append(["id","timestamp","cond_type","reading"])
    ...
    if fb_rows:
        ...
        ws4.append(["id","record_id","timestamp","rating"])
        for row in fb_rows:
            ws4.append(list(row))
...
def log_chronic(user_id, cond_type, reading):
    conn = _conn()
    try:
        c = conn.cursor()
        c.execute(
            f"INSERT INTO chronic_logs (user_hash, cond_type, reading, timestamp) VALUES ({PH},{PH},{PH},{PH})",
            (_hash_user(user_id), cond_type, reading, datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
    finally:
        conn.close()
...
def _all_chronic_logs():
    conn = _conn()
    try:
        c = conn.cursor()
        c.execute("SELECT id, timestamp, cond_type, reading FROM chronic_logs ORDER BY id")
        return c.fetchall()
    finally:
        conn.close()


def save_feedback(user_id, record_id, rating):
    conn = _conn()
    try:
        c = conn.cursor()
        c.execute(
            f"INSERT INTO feedback (user_hash, record_id, rating, timestamp) VALUES ({PH},{PH},{PH},{PH})",
            (_hash_user(user_id), record_id, rating, datetime.now(timezone.utc).isoformat()),
        )
        conn.commit()
    finally:
        conn.close()
...
def _all_feedback():
    ...
    try:
        c = conn.cursor()
        c.execute("SELECT id, record_id, timestamp, rating FROM feedback ORDER BY id")
        return c.fetchall()
    finally:
        conn.close()
